  ...
  <div class="app-container">
    ...
    >
      ...
      <jaya-upload-file
        ...
        :multiple="true"
        fileTitle="地图文件"
        annotation="*注：科目二：gst,dat,id,map,tab，科目三：mdb 示例：map.mdb"
      >
      </jaya-upload-file>
      <jaya-upload-file
        ref="upload"
...
export default {
  ...
  methods: {
    ...
    getBrandSelect() {
      ...
      brandSelect({ carType, drvSchoolId, trainSubject,equipType:2 }).then((res) => {
    ...
    getFileTrackList(fileList) {
      this.importFileTrack = fileList.map((item) => {
        return item.raw;
      });
    },
    // 提交
    handleUpload(formName) {
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          ...
          this.importFileTrack?.forEach((item, index) => {
            params["gpsTrackMapFiles[" + index + "]"] = item;
          });
