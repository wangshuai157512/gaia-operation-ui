    ...
    >
      ...
      >
        ...
        <el-form-item
          label="参数值"
          prop="csz"
          v-if="form.val && form.val.indexOf('\\') !== -1"
        >
          <el-select v-model="form.val" placeholder="请选择" size="small">
            <el-option
        ...
        <el-form-item
          ...
          prop="val"
        >
          <el-input v-model="form.val" placeholder="请输入" size="small">
          </el-input>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      csfwList: [],
      rules: {
        val: [{ required: true, message: "必填项", trigger: "change" }]
      },
      openAdd: false,
  ...
  methods: {
    ...
    edit() {
      ...
      this.isEdit = true;
      this.openAdd = true;
      this.form = { ...this.form, ...this.activeRow };
    ...
    handleEdit(formName) {
      this.$refs.form.validate(async (valid) => {
        if (valid) {
          let { id, val } = this.form;
          vehicleConfigEdit({ id, val, equipType: 2 }).then((res) => {
            this.$message({
              message: "编辑成功",
              type: "success"
            });
    ...
    async handleUpload() {
      ...
      formdata.append("equipType", 2);
      let { code } = await vehicleUploadConfigYxt(formdata);
      this.$message({
        message: "上传成功",
        type: "success"
      });
