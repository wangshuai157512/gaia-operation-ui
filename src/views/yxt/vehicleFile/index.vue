...
<script>
import { addVehicleFile, vehicleListYxt } from "@/api/cloud/configuration";
export default {
  name: "vehicleFile",
  data() {
    return {
  ...
  methods: {
    getTableList(num) {
      ...
      vehicleListYxt(this.queryParams).then((res) => {
    ...
    edit() {
      ...
      this.isEdit = true;
      let { drvSchoolId, id,trainSubject } = this.activeRow;
    ...
    handleUpload(formName) {
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          ...
          let { drvSchoolId, id, trainSubject } = this.form;
          let params = {
            drvSchoolId,
            id,
            trainSubject,
