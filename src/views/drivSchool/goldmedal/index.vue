      ...
      <div class="custom-dialog-con">
        ...
        <div class="formShowInfo">
          ...
          >
            ...
            <el-form-item label="车辆编号" prop="id" v-if="operating == 3">
              <el-input
                v-model="form.id"
                size="small"
                placeholder="系统自动生成"
            ...
            <el-form-item label="使用科目" prop="subject" v-if="operating === 3">
              <el-select
            ...
            <el-form-item label="安装设备日期" size="small" prop="installTime" v-if="operating==3">
              <el-date-picker
  ...
  data() {
    return {
      ...
      rules: {
        ...
        drvSchoolId: [{ required: true, trigger: "change", message: "必填项" }],
        status: [{ required: true, trigger: "change", message: "必填项" }],
        carModel: [{ required: true, trigger: "change", message: "必填项" }],
        carBrand: [{ required: true, trigger: "change", message: "必填项" }],
        equipType: [{ required: true, trigger: "change", message: "必填项" }],
      },
    };
  },
  mounted() {
  ...
  methods: {
    ...
    deleteClick() {
      if (this.multipleSelection) {
        const { id, equipType } = this.multipleSelection;
        this.$confirm("是否确认删除?", "警告", {
          ...
          .then(function () {
            return pageRemove({ id, equipType });
          })
          .then(() => {
            this.getTableList();
    ...
    updateShow(row) {
      ...
      if (this.multipleSelection) {
        const { id, equipType } = this.multipleSelection;
        pageInfo({ id, equipType }).then(({ data }) => {
          data.carPurpose = data.carPurpose ? data.carPurpose + "" : "";
