    ...
    <div class="nav-card">
      <div class="grid-content nav-card-item">
        ...
        <div class="card-item-amount">
          <countTo class="amount-t" :endVal="total" :duration="durationTime"/>
          <span>/人</span>
        </div>
      </div>
      <div class="grid-content nav-card-item">
        ...
        <div class="card-item-amount">
          <countTo class="amount-t" :endVal="orderNum" :duration="durationTime" />
          <span>/人</span>
        </div>
      </div>
      <div class="grid-content nav-card-item">
...
export default {
  ...
  mounted() {
    ...
    let dataAllMoney = [
      600.86, 700.41, 900.02, 800.86, 658.88, 728.14, 785, 635.26, 925, 755.32, 710.82,
      412.62,
    ];
    this.drawBar(date, dataAllMoney);
    ...
    this.drawPieIncome(comeData);
    Timer = setInterval(()=> {
      if(this.durationTime!==0.5) {
        this.durationTime = 0.5
      }
      let num = Math.floor(Math.random() * 10)
      this.total = this.total+num
      this.orderNum = this.orderNum+num
      traintimeData = traintimeData.map(item=>{
        return {
          ...item,
          value:(item.value*1000+num*1000)/1000
        }
      })
      this.drawPie(traintimeData)
      comeData = comeData.map(item=>{
        return {
          ...item,
          value:(item.value*1000+num*1000)/1000
        }
      ...
      const option = {
        ...
        legend: {
          ...
          formatter: function (name) {
            ...
            return name + " " + " " + p.toFixed(2) + "%" + " " + "￥" + val.toFixed(2);
          },
        },
