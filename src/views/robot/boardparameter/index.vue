    ...
    >
      ...
      >
        ...
        <el-form-item label="设备名称" prop="equipmentName">
          <el-input
            v-model="reserveForm.equipmentName"
            placeholder="请输入设备名称"
          />
        </el-form-item>
        ...
        <el-form-item label="状态" class="reserveFormstate" prop="state">
          <el-select
            size="small"
...
export default {
  ...
  methods: {
    ...
    // 查看api
    async getInfo(id) {
      await getInfoApi({ id: id,equipType:1 })
        .then((res) => {
          if (res.code == 200) {
    ...
    removeAction(row) {
      this.$confirm(`确定要删除该车载设备`, "提示", {
        ...
        let { data } = await removeApi({ id: row.id,equipType:1 });
        this.getTableList(null);
        this.$message({
          type: "success",
          message: "删除成功!",
    ...
    save() {
      this.$refs.reserveForm.validate((valid) => {
        if (valid) {
          ...
          saveapi({...this.reserveForm,equipType:1})
            .then((res) => {
              if (res.code == 200) {
                this.cancel();
                this.getTableList(null);
